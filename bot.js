require("dotenv").config()   
   ...
   // ---------------------------------------------------------
   // PNL (Profit and Loss) functionality - NEW ADDITION
   // ---------------------------------------------------------
   async function calculatePnL(walletPublicKey) {
     try {
       logger.info(`Calculating PNL for wallet: ${walletPublicKey}`)
       const c = new Connection(SOLANA_RPC_URL, "confirmed")
       // Fetch recent transactions (up to 100) for the wallet
       ...
       const timeStats = {
         "24h": { trades: 0, profit: new Decimal(0), loss: new Decimal(0) },
         "7d": { trades: 0, profit: new Decimal(0), loss: new Decimal(0) },
         "30d": { trades: 0, profit: new Decimal(0), loss: new Decimal(0) },
         "all": { trades: 0, profit: new Decimal(0), loss: new Decimal(0) }
       }
       ...
       let totalSpentSol = new Decimal(0)
       let totalEarnedSol = new Decimal(0)
       let winningTrades = 0
       let losingTrades = 0
       for (const sig of signatures) {
         try {
           if (sig.err) continue // Skip failed transactions
           const txData = await c.getTransaction(sig.signature, {
             commitment: "confirmed",
             maxSupportedTransactionVersion: 0
           })
           if (!txData || !txData.meta) continue
           const timestamp = txData.blockTime || 0
           const txFee = new Decimal(txData.meta.fee).div(1_000_000_000)
           // Check pre and post token balances to identify swaps
           const preTokenBalances = txData.meta.preTokenBalances || []
           const postTokenBalances = txData.meta.postTokenBalances || []
           // If this appears to be a swap transaction
           if (preTokenBalances.length > 0 && postTokenBalances.length > 0) {
             // Try to determine if this was a buy or sell
             let isBuy = false
             let isSell = false
             ...
             let tokenMint = ""
             // Check if SOL was involved in the transaction
             for (const balance of preTokenBalances.concat(postTokenBalances)) {
               if (balance.mint === "So11111111111111111111111111111111111111112") {
                 // This involves SOL
                 const delta = calculateTokenDelta(preTokenBalances, postTokenBalances, balance.mint)
                 if (delta.gt(0)) {
                   isSell = true // User received SOL
                   solAmount = delta
                 } else if (delta.lt(0)) {
                   isBuy = true // User spent SOL
                   solAmount = delta.abs()
                 }
               } else if (balance.mint) {
                 // Store token info
                 tokenMint = balance.mint
                 const delta = calculateTokenDelta(preTokenBalances, postTokenBalances, balance.mint)
                 tokenAmount = delta.abs()
                 // Try to get token symbol from aggregator
                 try {
                   const tokenInfo = await getTokenInfoFromAggregator(balance.mint)
                   if (tokenInfo && tokenInfo.symbol) {
                     tokenSymbol = tokenInfo.symbol
                   }
                 } catch (e) {
                   logger.error(`Error getting token info for ${balance.mint}:`, e.message)
                 }
               }
             }
             ...
             if (isBuy || isSell) {
               ...
               if (isBuy) {
                 totalSpentSol = totalSpentSol.add(solAmount).add(txFee)
               } else if (isSell) {
                 totalEarnedSol = totalEarnedSol.add(solAmount).sub(txFee)
                 // Consider it a winning trade if earned > spent
                 if (solAmount.gt(totalSpentSol)) {
                   winningTrades++
                   // Update time-based statistics
                   const profit = solAmount.sub(totalSpentSol)
                   timeStats.all.profit = timeStats.all.profit.add(profit)
                   timeStats.all.trades++
                   if (timestamp >= oneDayAgo) {
                     timeStats["24h"].profit = timeStats["24h"].profit.add(profit)
                     timeStats["24h"].trades++
                   }
                   if (timestamp >= sevenDaysAgo) {
                     timeStats["7d"].profit = timeStats["7d"].profit.add(profit)
                     timeStats["7d"].trades++
                   }
                   if (timestamp >= thirtyDaysAgo) {
                     timeStats["30d"].profit = timeStats["30d"].profit.add(profit)
                     timeStats["30d"].trades++
                   }
                 } else {
                   losingTrades++
                   // Update time-based statistics for losses
                   const loss = totalSpentSol.sub(solAmount)
                   timeStats.all.loss = timeStats.all.loss.add(loss)
                   timeStats.all.trades++
                   if (timestamp >= oneDayAgo) {
                     timeStats["24h"].loss = timeStats["24h"].loss.add(loss)
                     timeStats["24h"].trades++
                   }
                   if (timestamp >= sevenDaysAgo) {
                     timeStats["7d"].loss = timeStats["7d"].loss.add(loss)
                     timeStats["7d"].trades++
                   }
                   if (timestamp >= thirtyDaysAgo) {
                     timeStats["30d"].loss = timeStats["30d"].loss.add(loss)
                     timeStats["30d"].trades++
                   }
                 }
                 // Reset spent after calculating profit/loss
                 totalSpentSol = new Decimal(0)
               }
             }
           }
         } catch (e) {
           logger.error(`Error processing transaction ${sig.signature}:`, e.message)
           continue
         }
       }
       // Calculate overall ROI
       const netProfit = totalEarnedSol.sub(totalSpentSol)
       const roi = totalSpentSol.isZero() ? new Decimal(0) : netProfit.div(totalSpentSol).mul(100)
       // Calculate win rate
       ...
       return {
         ...
         roi: roi.toFixed(2),
         totalSpentSol: totalSpentSol.toFixed(4),
         totalEarnedSol: totalEarnedSol.toFixed(4),
         recentTransactions: transactionDetails.slice(0, 10), // Return most recent 10 transactions
         timeStats
       }
     } catch (e) {
       logger.error("calculatePnL error:", e)
       return {
         success: false,
         message: "Failed to calculate PNL: " + e.message
       }
     }
   }
   // Helper function to calculate token balance delta between pre and post token balances
   function calculateTokenDelta(preBalances, postBalances, tokenMint) {
     let preAmount = new Decimal(0)
     let postAmount = new Decimal(0)
     for (const balance of preBalances) {
       if (balance.mint === tokenMint) {
         preAmount = new Decimal(balance.uiTokenAmount.uiAmount || 0)
       }
     }
     for (const balance of postBalances) {
       if (balance.mint === tokenMint) {
         postAmount = new Decimal(balance.uiTokenAmount.uiAmount || 0)
       }
     }
     return postAmount.sub(preAmount)
   }
   // Function to show PNL menu
   async function showPnLMenu(chatId, messageId = null) {
     try {
       ...
       if (!u || !u.public_key) {
         if (messageId) {
           await editMessageText(
             chatId, 
             messageId, 
             "⚠️ You need to connect a wallet first to view PNL stats.", 
       ...
       if (messageId) {
         await editMessageText(
           chatId,
           messageId,
           "🔄 Calculating PNL from blockchain transactions...\nThis may take a moment...",
           {}
         )
       ...
       // Get PNL stats from blockchain
       const pnlData = await calculatePnL(u.public_key)
       if (!pnlData.success) {
         await editMessageText(
           chatId,
           msg.message_id,
           `⚠️ ${pnlData.message || "Failed to calculate PNL."}`,
           { inline_keyboard: [[{ text: "« Back", callback_data: "BACK_MAIN" }]] }
         )
         return
       }
       // Format the message
       let pnlMessage = `📊 *Profit & Loss Summary*\n\n`
       // Add time-based statistics
       pnlMessage += `📅 *Time Period Stats*\n`
       pnlMessage += `• 24h: ${pnlData.timeStats["24h"].trades} trades | Profit: ${pnlData.timeStats["24h"].profit.toFixed(4)} SOL | Loss: ${pnlData.timeStats["24h"].loss.toFixed(4)} SOL\n`
       pnlMessage += `• 7d: ${pnlData.timeStats["7d"].trades} trades | Profit: ${pnlData.timeStats["7d"].profit.toFixed(4)} SOL | Loss: ${pnlData.timeStats["7d"].loss.toFixed(4)} SOL\n`
       pnlMessage += `• 30d: ${pnlData.timeStats["30d"].trades} trades | Profit: ${pnlData.timeStats["30d"].profit.toFixed(4)} SOL | Loss: ${pnlData.timeStats["30d"].loss.toFixed(4)} SOL\n\n`
       // Overall stats
       pnlMessage += `🎯 *Overall Trading Stats*\n`
       pnlMessage += `• Win Rate: ${pnlData.winRate}%\n`
       pnlMessage += `• Total Trades: ${pnlData.totalTrades}\n`
       pnlMessage += `• Winning Trades: ${pnlData.winningTrades}\n`
       pnlMessage += `• Losing Trades: ${pnlData.losingTrades}\n\n`
       pnlMessage += `💰 *Financial Summary*\n`
       pnlMessage += `• Net Profit: ${pnlData.netProfit} SOL\n`
       pnlMessage += `• ROI: ${pnlData.roi}%\n`
       pnlMessage += `• Total Spent: ${pnlData.totalSpentSol} SOL\n`
       pnlMessage += `• Total Earned: ${pnlData.totalEarnedSol} SOL\n\n`
       // Recent transactions
       if (pnlData.recentTransactions && pnlData.recentTransactions.length > 0) {
         pnlMessage += `🔄 *Recent Transactions*\n`
         for (let i = 0; i < Math.min(5, pnlData.recentTransactions.length); i++) {
           const tx = pnlData.recentTransactions[i]
           const date = new Date(tx.timestamp * 1000).toLocaleString()
           const type = tx.type === "BUY" ? "🟢 Buy" : "🔴 Sell"
           const txShort = tx.signature.substring(0, 8) + "..."
           pnlMessage += `• ${type}: ${tx.solAmount} SOL ${tx.type === "BUY" ? "→" : "←"} ${tx.tokenAmount} ${tx.tokenSymbol} (${date})\n`
         }
       }
       await editMessageText(
         chatId,
       ...
       logger.error("showPnLMenu error:", e)
       if (messageId) {
         await editMessageText(
           chatId,
           messageId,
           "❌ Error calculating PNL. Please try again later.",
           { inline_keyboard: [[{ text: "« Back", callback_data: "BACK_MAIN" }]] }
         )
       } else {
         await bot.sendMessage(
   ...
   function mainMenuKeyboard(autoTradeEnabled) {   
     ...
     return {   
       inline_keyboard: [   
         ...
         [   
           { text: "📈 PnL Stats", callback_data: "PNL_MENU" },   
         ],   
       ],   
     }   
   }   
   ...
   - *📈 PnL Stats*: View your profit/loss statistics     
   ⚠️ *Trading Tips*     
   - Default slippage: 1%    
   - Failed swap? Check:     
   ...
   *Balance:* ${sb.toFixed(4)} SOL (${userSolBalUsd.toFixed(2)})   
   ...
   - *🤖 Auto-Trade*: Allocate SOL for priority access to new launches     
   - *💸 Withdraw*: Send SOL to external wallets     
   - *📈 PnL Stats*: Track your profit and loss for trades     
   ⚠️ *Trading Tips*     
   - Default slippage: 1%    
   - Failed swap? Check:     
   - Enough SOL for gas + amount     
   ...
   *Pro Tip:* Use /buy [mint] or /sell [amount] for quick actions!   
   ...
   bot.onText(/\/pnl/, async (msg) => {   
     try {   
       ...
       const u = await getUserRow(chatId)
       if (!u || !u.public_key) {
         return bot.sendMessage(chatId, "No wallet found. Please /start => create or import one first.")
       }
       await showPnLMenu(chatId)
     } catch (err) {   
       logger.error("/pnl command error:", err)   
       await bot.sendMessage(msg.chat.id, "Error processing PNL request. Please try again.") 
     }   
   })
